diffPlot.formula <- function(formula,data, plotFUN=mean, errFUN=c("ci", "se", "sd"), conf=.95, grp.names=NULL, var.equal=FALSE, paired=FALSE, ...) {
  if(paired==TRUE) warning("You said paired=TRUE, but provided formula. Paired argument is ignored. For paired data, use 'x, y' notation.")
  se <- function(x) {
    x <- na.omit(x)
  ...
  dat <- na.omit(dat)
  if(length(unique(dat[,2])) != 2) {stop("Grouping factor must have exactly two levels.")}
